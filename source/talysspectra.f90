...
!
  ...
  character(len=80) :: Astring     ! angle string
            ...
            if (abs(Ein-Efile) <= 1.e-4) then
              ...
              if (flagbreakup) then
                do nen2 = 1, ncumout(type,nen)
                  read(1, '(f8.3,e12.5,48x,2e12.5)') Eocum(type,nen,nen2) , xsemis(type,nen,nen2), &
 &                  preeqratio(type,nen,nen2), buratio(type,nen,nen2)
                enddo
              else
                do nen2 = 1, ncumout(type,nen)
                  read(1, '(f8.3,e12.5,48x,e12.5)') Eocum(type,nen,nen2), xsemis(type,nen,nen2), preeqratio(type,nen,nen2)
                enddo
              endif
              exit
            else
      ...
      do nen = 1, Nenspec
        ...
        do iang = 0, 180
          ...
          if (lexist) then
            ...
            do
              read(1,'(/,a,/,a,/14x,i6/)', iostat = istat) Estring, Astring, Nfile
              if (istat == -1) exit
              if (flagblock) then
                read(Estring(16:80), *) Efile
                read(Astring(16:80), *) Afile
                if (abs(Ein-Efile) <= 1.e-4 .and. abs(ang-Afile) <= 1.e-4) then
                  ...
                  do nen2 = 1, ncumddx(type,nen,iddx)
                    read(1, '(f8.3,e12.5)') Eoddx(type,nen,iddx,nen2), ddxemis(type,nen,iddx,nen2)
                  enddo
                  exit
                else
                  do nen2 = 1, Nfile
