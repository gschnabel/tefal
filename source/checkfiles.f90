...
!
  if (endftext(1:1) == ' ') then
    if (flaggpf) then
      if (k0 == 0) endftext = trim(path)//'aux/endf_g.txt'
      if (k0 == 1) endftext = trim(path)//'aux/endf_n.txt'
      if (k0 > 1) endftext = trim(path)//'aux/endf_p.txt'
    else
      if ( .not. flaggpf .and. .not. flageaf) then
        if (k0 == 0) endftext = trim(path)//'aux/endf_act_g.txt'
        if (k0 == 1) endftext = trim(path)//'aux/endf_act_n.txt'
        if (k0 > 1) endftext = trim(path)//'aux/endf_act_p.txt'
      endif
    endif
  else
